...
3. Open Capture from Home, Units, and Issues on Mac, iPhone, and iPad; confirm voice mode starts immediately without route change or keyboard zoom.
4. Use Type, Camera, Photo, and File from the voice surface; confirm each returns to the same draft-first Capture workspace.
5. Type one multi-unit note and confirm each proposed action is separate.
6. Approve one action and reject one action; confirm Home and the board reflect only the approved action.
7. Take or attach one work-safe photo, confirm the suggested Unit, and save it.
8. Confirm More traps focus, closes with Escape, and restores focus to its trigger.
