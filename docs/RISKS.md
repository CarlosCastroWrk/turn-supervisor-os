...
| Same-day Daily Logs created offline could violate the cloud project/date uniqueness rule | High | Guarded | The current candidate uses deterministic new IDs, tuple-aware local save, legacy cloud-ID reconciliation, all six reconnect-order tests, and a 5,000-log performance gate. Production release and physical-device acceptance remain pending. |
