...
- Started D7 Capture-First Field Shell on `codex/capture-first-field-shell` from clean `main` after Los approved the next UI release and confirmed production H2 metering was visible.
- Reframed the visible product around Home, Units, Issues, Crew, Reports, Setup, and universal Capture while keeping Assignments, Daily Log, Memory, and backup behavior available through their existing automation or secondary routes.
- Replaced the old Dashboard quick-action grid with compact readiness, tappable status counts, Needs attention, today's movement, and next-action sections; approved Capture changes surface in Home activity immediately.
- Made one Capture tap open the voice surface automatically with Camera, Photo, File, and Type fallbacks, while retaining draft-first review and preventing keyboard autofocus on iPhone/iPad.
- Added a raised mobile Capture action, a contained More sheet, true left-edge desktop navigation, and generated desktop/mobile design references for the release review. No migration, environment change, provider request, or production data change is part of this slice.
- The rendered Capture gate caught and fixed three cross-device focus defects: Finish could strand focus on the document body, mobile Capture dismissal targeted the hidden desktop trigger, and More dismissal could race the inert background. The gate now verifies layered dismissal, visible-trigger restoration, focus wrapping, and background isolation.
- Visual concept comparison caught overlapping Capture/voice opacity animations that briefly washed out the iPhone surface. Removed the nested fade and verified settled opaque iPhone/iPad Capture renders before saving implementation screenshots with the design references.
- Passed 218/218 deterministic tests, zero dependency vulnerabilities, Capture workflow QA with a staged/saved Unit photo, a 1,000-Unit/10,000-event field-scale run, durable 5,000-Unit import, 300-Unit bulk update, and the seven-export local recovery gate with corrupt/canceled/valid restore coverage.
- Merged PR #66 at `78f6d6f`, deployed D7 as `dpl_8UR8yhHBzYh4A5ht4G5inoMrQfJ4`, and confirmed the canonical URL serves the reviewed CSS/JS bundles with the existing CSP and camera/microphone policy.
- Passed a fresh unsigned production smoke on desktop, iPad, and iPhone sizes: Home and one-tap voice Capture rendered without overflow, the service worker was active, Capture open/close made zero model requests and no AppData change, and consoles/requests had zero findings. No migration, environment change, provider request, authentication, sync upload, or production data mutation occurred.
- Started D8 Board-First Field Pages on `codex/board-first-field-pages` from clean `main` after Los identified that manual Issue and Crew forms were crowding the field workflow and that Unit detail buried the practical checks, photos, and quick actions.
- Reordered Issues and Crew around their boards/directories, moving manual entry into accessible, focus-contained dialogs. The Issue board now exposes only In Progress and Done as field-facing states while preserving internal history and confirmed soft-close behavior.
- Reordered Unit detail around current checks, an editable bed count, fast actions, quick note, photos, and linked issues. Full manual controls remain available but collapsed; room/common-area completion is intentionally not invented before training clarifies the real checklist.
- Added shared-completion parsing so a note such as “Unit 104, Unit 106, Unit 210 are done” becomes one separate Ready draft per named Unit, with existing confirmation and Ready-safety checks still applied.
- Passed lint, production build, 219 deterministic tests, Browser-plugin desktop interaction QA, and a new desktop/iPad/iPhone board-first Playwright gate with no overflow or runtime findings. Deployment and physical-device acceptance remain pending.
- Merged PR #68 at `70327a9`, deployed D8 as `dpl_BApbYbhWvAo7jFZ4jH3uqrEgsw6j`, and confirmed the canonical URL aliases the Ready deployment.
- Passed a read-only production smoke: Issues opens on the board with no manual dialog, Unit detail surfaces checks, beds, actions, quick note, photos, and collapsed manual details, the service worker is registered, and an isolated browser reported no console or page errors. Physical Mac/iPhone/iPad acceptance remains open.
- Completed the P0 Account-Scoped Cache Guard plus I1 Grounded Turn Pulse and I2 Turn OS Playbooks as three reviewable stacked slices.
- Bound the device-local cache to one Supabase account, pinned sync work to its starting session, paused record/photo requests at `Cache needs review` for unbound or foreign caches, and required explicit confirmation before a matching account can claim local records.
- Added a deterministic active-Turn Pulse with one evidence-backed Start Here action and three static Playbooks for Capture/Confirm, blocker handling, and closing the day; all guidance remains personal, read-only, draft-first, and explicitly not official company procedure.
- Passed 234/234 deterministic tests, lint, build, OS checks, the headless and visible Mac account-switch gates, Turn Pulse and Playbooks browser gates, protected-preview review, and desktop/iPhone production rendering without new runtime findings.
- Merged PRs #70, #71, and #72 in order at `d506af5`, `8dedc60`, and `b6d055c`; Vercel deployed the final production commit as `dpl_7QzXf9h1xZmTPidXFNvWXCFYGU7t` with the canonical alias and reviewed security headers.
- Refreshed the existing PWA shell without clearing local data, confirmed live Turn Pulse and Playbooks on desktop and 390-by-844 mobile, and left the current device safely paused at `Cache needs review`. No cache claim, Supabase migration, secret change, provider request, or production data mutation was performed.
