...
| Demo data could contaminate Real Turn reports, Capture history, Copilot answers, or exports | High | Guarded | The Active Project Boundary release candidate adds project-proven drafts, fail-closed approval, current-Turn human-readable exports, explicit-project report selectors, and local-only handling for Demo/unverifiable Copilot rows. Production release and physical-device sentinel checks remain pending. |
