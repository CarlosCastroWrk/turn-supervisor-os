...
- Home dashboard, setup, units, unit detail, issues, crews, assignments, daily log, reports, training questions, and export views
...
- Universal Capture through a raised iPhone bottom action and fixed desktop action, with organized/collapsible navigation on larger screens
...
3. Confirm a Unit opens with checks, bed count, quick actions, quick note, photos, and linked issues before Manual unit details.
4. Open Capture from Home, Units, and Issues on Mac, iPhone, and iPad; confirm voice mode starts immediately without route change or keyboard zoom.
5. Type “Unit 101, Unit 102 are done” and confirm it produces two separate draft actions; approve only a disposable Demo update.
6. Take or attach one work-safe photo, confirm the suggested Unit, and save it.
7. Repeat once offline, then reconnect and confirm the approved records and saved photo sync normally.
8. Separately finish the still-open physical CSV, bulk update, and Home Screen restart checks.
