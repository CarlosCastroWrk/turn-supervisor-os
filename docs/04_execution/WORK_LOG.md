...
- Prepared `20260710133819_add_ai_usage_metering.sql` for owner-scoped usage rows and project budgets. It has not been applied and the branch must not merge until Los gives fresh explicit migration approval.
- Stress testing caught two model-boundary defects before release: “leaking” under-routed a mixed-workflow note to nano, and a model could reinterpret “ask Tony” as an unnamed crew assignment. The classifier now routes that note to mini, and conversion rejects assignments without explicit move/assign language plus a named or known crew.
- The final synthetic mini call returned a maintenance issue and Tony follow-up with 1,047 input tokens, 573 output tokens, and a $0.00336375 estimate. No real field data was sent, and no further model calls are needed for this slice.
...
- Supabase dry-run lists only `20260710133819_add_ai_usage_metering.sql`; no migration was applied. The current linked schema has no lint errors, and the pending migration now requires both usage-row ownership and referenced-project ownership.
