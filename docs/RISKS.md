...
| Demo data could contaminate Real Turn reports or Copilot answers | High | Guarded | Project filters, the Demo sync boundary, project-scoped Memory consumption, and project-only Daily Log history now block known cross-project paths. Continue physical-device checks of Real Turn exports, reports, and sync with obvious demo vs real labels. |
